# Claude Code Project Instructions — rcgen-kotlin

## Project Overview

This is **rcgen-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rcgen`](https://crates.io/crates/rcgen).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rcgen/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rcgen>
